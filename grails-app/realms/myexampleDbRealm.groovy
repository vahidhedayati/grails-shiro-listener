...
class myexampleDbRealm {
    ...
    def authenticate(authToken) {
        ...
        // exception.
        def user = myexampleUser.findByUsername(username)
        if (!user) {
        ...
        // in the database.
        def account = new SimpleAccount(username, user.passwordHash, "myexampleDbRealm")
    ...
    def hasRole(principal, roleName) {
        def roles = myexampleUser.withCriteria {
            roles {
                eq("name", roleName)
            }
    ...
    def hasAllRoles(principal, roles) {
        def r = myexampleUser.withCriteria {
            roles {
                'in'("name", roles)
            }
            eq("username", principal)
    ...
    def isPermitted(principal, requiredPermission) {
        ...
        def user = myexampleUser.findByUsername(principal)
        ...
        def results = myexampleUser.executeQuery("select distinct p from myexampleUser as user join user.roles as role join role.permissions as p where user.username = '$principal'")
